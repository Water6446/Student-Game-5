-- =============================================================================
-- 0029_hidden_odds.sql — "hide the odds from students" actually hides them;
-- and a round can be revealed only once.
--
-- 1. HIDDEN ODDS WERE ONLY HIDDEN BY THE UI.
--    show_odds_to_students=false stopped the student screens DRAWING good_prob,
--    but good_prob stayed in sessions.config, which every session member can
--    read (0002). A student with the network tab open saw the exact odds the
--    host had hidden — the "curious student with dev tools" 0002 is written
--    against. 0014 solved the same problem for manager alphas with
--    session_secrets; hidden odds now live there too, under secret->'odds'.
--
--    One BEFORE UPDATE trigger keeps it true on every path:
--      * odds hidden and the game not finished -> any good_prob (game-level or
--        per portfolio asset) arriving in config is moved into session_secrets
--        and stripped from the row before it is written. set_good_prob and
--        set_show_odds need no changes: whatever they write is caught here.
--      * odds shown, or the game finished -> stashed odds are put back into
--        config. Students see them once the game is over, like manager truth.
--    A row can be INSERTED with odds (create_session is ~290 lines and stays
--    untouched); an AFTER INSERT trigger re-saves it so the update trigger can
--    move them. Nobody but the host is a member of a session at that moment.
--
--    "Hidden" means show_odds_to_students is not literally true, matching
--    every student screen (`config.show_odds_to_students && …`). Manager games
--    have no good/bad odds and are skipped.
--
--    The host reads stashed odds through get_hidden_odds(); resolve_round
--    merges them back in before it reads the config (the only other change to
--    its body below). lib/game/hidden-odds.ts mirrors _apply_odds for the host
--    screens.
--
-- 2. TWO REVEALS OF ONE ROUND.
--    resolve_round read the round without locking it. Two reveals at the same
--    moment (the host's laptop and tablet, or two tabs) both saw 'locked', both
--    resolved, and the second re-rolled the market after students had already
--    seen the first result. The round row is now read FOR UPDATE: the second
--    call waits, then finds 'revealed' and refuses.
--
-- resolve_round below is 0015's body with exactly those two changes, each
-- marked "0029". It is the definition from here on.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- Odds helpers. Internal: called by triggers and SECURITY DEFINER functions.
-- ---------------------------------------------------------------------------

-- Are this config's odds hidden from students right now?
create or replace function public._odds_hidden(p_config jsonb, p_status text)
returns boolean
language sql immutable set search_path = public, pg_temp as $$
  select coalesce(p_config->>'game_type', 'basic') <> 'manager'
     and coalesce(p_status, 'lobby') <> 'finished'
     and coalesce(p_config->>'show_odds_to_students', '') <> 'true';
$$;

-- The odds a config carries: {good_prob?, assets?: [p | null, ...]}.
create or replace function public._odds_of(p_config jsonb)
returns jsonb
...
declare
  ...
  i        int;
begin
  if p_config ? 'good_prob' then
    v_out := jsonb_build_object('good_prob', p_config->'good_prob');
  end if;
  if jsonb_typeof(p_config->'assets') = 'array' then
    for i in 0 .. jsonb_array_length(p_config->'assets') - 1 loop
      v_asset := p_config->'assets'->i;
      if jsonb_typeof(v_asset) = 'object' and v_asset ? 'good_prob' then
        v_assets := v_assets || jsonb_build_array(v_asset->'good_prob');
        v_any := true;
      else
        v_assets := v_assets || jsonb_build_array(null::jsonb);
      end if;
    end loop;
    if v_any then
      v_out := v_out || jsonb_build_object('assets', v_assets);
    end if;
  end if;
  return v_out;
end;
...
-- The config with every good_prob removed (game-level and per asset).
create or replace function public._strip_odds(p_config jsonb)
returns jsonb
language plpgsql immutable set search_path = public, pg_temp as $$
declare
  v_out    jsonb := p_config - 'good_prob';
  v_assets jsonb := '[]'::jsonb;
  v_asset  jsonb;
  i        int;
begin
  if jsonb_typeof(v_out->'assets') = 'array' then
    for i in 0 .. jsonb_array_length(v_out->'assets') - 1 loop
      v_asset := v_out->'assets'->i;
      v_assets := v_assets || jsonb_build_array(
        case when jsonb_typeof(v_asset) = 'object' then v_asset - 'good_prob' else v_asset end);
    end loop;
    v_out := jsonb_set(v_out, '{assets}', v_assets);
  end if;
  return v_out;
end;
$$;
...
-- MIRRORS applyHiddenOdds in lib/game/hidden-odds.ts.
create or replace function public._apply_odds(p_config jsonb, p_odds jsonb)
returns jsonb
language plpgsql immutable set search_path = public, pg_temp as $$
declare
  v_out    jsonb := p_config;
  v_assets jsonb := '[]'::jsonb;
  v_asset  jsonb;
  v_odd    jsonb;
  i        int;
begin
  if p_odds is null or jsonb_typeof(p_odds) <> 'object' then
    return p_config;
  end if;
  if p_odds ? 'good_prob' and not (v_out ? 'good_prob') then
    v_out := v_out || jsonb_build_object('good_prob', p_odds->'good_prob');
  end if;
  if jsonb_typeof(p_odds->'assets') = 'array' and jsonb_typeof(v_out->'assets') = 'array' then
    for i in 0 .. jsonb_array_length(v_out->'assets') - 1 loop
      v_asset := v_out->'assets'->i;
      v_odd   := p_odds->'assets'->i;
      if jsonb_typeof(v_asset) = 'object' and not (v_asset ? 'good_prob')
         and v_odd is not null and jsonb_typeof(v_odd) <> 'null' then
        v_asset := v_asset || jsonb_build_object('good_prob', v_odd);
      end if;
...
-- The triggers.
-- ---------------------------------------------------------------------------
create or replace function public.sessions_hide_odds()
returns trigger
...
declare
  ...
  v_kept jsonb;
begin
  if coalesce(new.config->>'game_type', 'basic') = 'manager' then
    return new;  -- session_secrets holds the manager truth; never touch it here
  end if;

  select s.secret->'odds' into v_kept
    from public.session_secrets s where s.session_id = new.id;

  if public._odds_hidden(new.config, new.status) then
    v_odds := public._odds_of(new.config);
    if v_odds <> '{}'::jsonb then
      -- What just arrived in config is newer than what was stashed.
      insert into public.session_secrets (session_id, secret)
      values (new.id, jsonb_build_object('odds', v_odds))
      on conflict (session_id) do update
        set secret = public.session_secrets.secret || jsonb_build_object('odds',
              coalesce(public.session_secrets.secret->'odds', '{}'::jsonb) || v_odds);
      new.config := public._strip_odds(new.config);
    end if;
  elsif v_kept is not null then
    new.config := public._apply_odds(new.config, v_kept);
    update public.session_secrets set secret = secret - 'odds' where session_id = new.id;
    delete from public.session_secrets where session_id = new.id and secret = '{}'::jsonb;
  end if;
...
create or replace function public.sessions_hide_odds_on_insert()
returns trigger
language plpgsql security definer set search_path = public, pg_temp as $$
begin
  if public._odds_hidden(new.config, new.status)
     and public._odds_of(new.config) <> '{}'::jsonb then
    -- A BEFORE INSERT trigger cannot write session_secrets (its foreign key
    -- needs this row to exist yet), so save the row once more and let
    -- sessions_hide_odds move the odds.
    update public.sessions set config = new.config where id = new.id;
  end if;
  return null;
end;
$$;

drop trigger if exists sessions_hide_odds on public.sessions;
create trigger sessions_hide_odds
  before update on public.sessions
  for each row execute function public.sessions_hide_odds();

drop trigger if exists sessions_hide_odds_on_insert on public.sessions;
create trigger sessions_hide_odds_on_insert
  after insert on public.sessions
  for each row execute function public.sessions_hide_odds_on_insert();

-- Sessions already running with hidden odds: save each once so the trigger
-- moves its odds out of reach now.
update public.sessions
   set config = config
 where public._odds_hidden(config, status)
   and public._odds_of(config) <> '{}'::jsonb;

-- ---------------------------------------------------------------------------
-- get_hidden_odds — the host's copy. NULL for anyone else (and for sessions
-- with nothing hidden), so the shared useSession hook can ask without erroring.
-- ---------------------------------------------------------------------------
create or replace function public.get_hidden_odds(p_session_id uuid)
returns jsonb
language sql stable security definer set search_path = public, pg_temp as $$
  select s.secret->'odds' from public.session_secrets s
   where s.session_id = p_session_id
     and public.is_session_host(p_session_id);
$$;

revoke all on function public._odds_hidden(jsonb, text)          from public, anon, authenticated;
revoke all on function public._odds_of(jsonb)                    from public, anon, authenticated;
revoke all on function public._strip_odds(jsonb)                 from public, anon, authenticated;
revoke all on function public._apply_odds(jsonb, jsonb)          from public, anon, authenticated;
revoke all on function public.sessions_hide_odds()               from public, anon, authenticated;
revoke all on function public.sessions_hide_odds_on_insert()     from public, anon, authenticated;
revoke all on function public.get_hidden_odds(uuid)              from public, anon;
grant execute on function public.get_hidden_odds(uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- resolve_round — 0015's body; the two 0029 changes are marked.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- 0029: hidden odds live in session_secrets, not config. Resolve with them.
  v_session.config := public._apply_odds(v_session.config,
    (select s.secret->'odds' from public.session_secrets s where s.session_id = p_session_id));

  -- 0029: FOR UPDATE, so a second reveal of this round waits for the first,
  -- then sees 'revealed' and stops instead of re-rolling the market.
